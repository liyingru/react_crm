...
class ContactTab extends Component<ContactTabPros, ContactTabState>{
    ...
    render() {
        const { contactUserData, editable, config, permission, ...rest } = this.props;
        const { contact } = this.state;
        ...
        let phoneEditable = contact == undefined || permission?.updatecontactphone == true
            ...
            <div>
                {
                    editable && (
                        <div className={styles.newcontact}>
                            <Button size="small" onClick={this.newContact}><PlusOutlined />联系人</Button>
                        </div>
                    )
                }
                                ...
                                <FormItem label='角色'>
                                    {
                                        getFieldDecorator('identity', {
                                            initialValue: contact == undefined ? '' : contact.identity,
                                            rules: [{
                                                required: true,
                                                message: "角色不能为空"
                                            }]
                                ...
                                <FormItem label='电话'>
                                    {
                                        ...
                                        })(<Input placeholder="请输入" maxLength={11} disabled={!phoneEditable} />)
                                    }
                                </FormItem>
                                <FormItem label='微信号'>
                                    {
                                ...
                                <FormItem label='方便联系时间'>
                                    {
                                        getFieldDecorator('contactTime', {
                                            initialValue: contact == undefined ? '' : (contact.contactTime == 0 ? '' : contact.contactTime)
                                        })(
                                            <Select style={{ width: '100%' }} placeholder='请选择'>
                                                {
