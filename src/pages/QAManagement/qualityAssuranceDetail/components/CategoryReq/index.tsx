...
import { CategoryConfigItem, ConfigData, CustomerInfoData, RequirementData, RequirementDataDetails, Button, ReqButton } from '@/pages/LeadsManagement/leadsDetails/data';
import { RequirementDataGroupDetails } from '../../data'
...
import styles from '@/pages/DxlLeadsManagement/dxlLeadsList/style.less';
...
class CategoryReq extends Component<CategoryReqProps, CategoryReqState> {
  ...
  createCategoryReq(params: any, callback: (code: number, msg: string) => void) {
    ...
    Axios.post(URL.reqQaCreate, params).then(
      res => {
        if (res.code == 200) {
  ...
  tobeCategoryReq(params: any, callback: (code: number, msg: string) => void) {
    ...
    Axios.post(URL.reqQaPhase, params).then(
      res => {
        this.setState({
          optionLoading: false,
        })
  ...
  //创建有效单对话框确认
  handleCreateReqOk = () => {
    const { form, leadsId, customerId, config, reqId } = this.props;
  ...
  refreshTagReset = () => {
    localStorage?.setItem('qaListRefreshTag', 'reset')
  }
  ...
  handleButton = (record: RequirementData, button: ReqButton) => {
    ...
    if (button.type == 3) {  //有效
      this.handleValidReq(record)
    } else if (button.type == 4) {  //待定
      this.handleTobeReq(record)
    } else if (button.type == 5) {  //无效
      this.handleInvalidReq(record)
    } else if (button.type == 10) {  //日志
      this.handleOpenLogModal(record)
    } else if (button.type == 50) {  //无效原因
      this.handleVolidReqAlert(button.name, record)
    }
  }
  ...
  handleCloseReqOk = () => {
    ...
    if (this.state.closeModalType == 0 || this.state.closeModalType == 1) {
      ...
      form.validateFields(formFeilds, (err, values) => {
        ...
        const params = {
          invalidReason: values['closeRemark'],
          reqId: this.state.editReq.id,
          phase: '5',
        };
  ...
  handleRecommend = (data: RequirementData) => {
    this.props.fun_recommend(data.customer_id, data.id, data.category, data.city_info.city_code);
  }
  ...
  renderTable = (details: RequirementDataDetails, isHistory: boolean) => {
    /**
     * 当前支持类型1-7
     * 服务器增加类型涉及的改动
     * 1、该处放开到适定范围
    ...
    return (
      <Table
        ...
        columns={this.createColums(details.category_id, isHistory)}
  ...
  renderMyGroup = (reqDataList: RequirementDataDetails[]) => {
    return (
      <div>
        <div className={styles.headerTitle}>QA质检单</div>
        {
          reqDataList.map(details => (
            this.renderTable(details, false)
          ))
        }
      </div>
    );
  ...
  renderOtherGroup = (reqDataList: RequirementDataDetails[]) => {
    ...
    return (
      <div style={{ marginTop: maginTop }}>
        <div className={styles.headerTitle}>历史数据</div>
        {
          reqDataList.map(details => (
            this.renderTable(details, true)
          ))
        }
      </div>
    );
    ...
    return (
      <div>
        <Spin spinning={this.state.optionLoading}>
          ...
          {
            (reqGroupDetails.req_qa_data && reqGroupDetails.req_qa_data.length > 0 && reqGroupDetails.req_qa_data[0].category_id.toString() == this.state.categoryId) &&
            this.renderMyGroup(reqGroupDetails.req_qa_data)
          }
          {
            (reqGroupDetails.req_data && reqGroupDetails.req_data.length > 0) && reqGroupDetails.req_data[0].category_id.toString() == this.state.categoryId &&
            this.renderOtherGroup(reqGroupDetails.req_data)
          }
        </Spin>
      </div >
    );
 ...
 */
   ...
   */
  createColums = (categoryId: number, isHistory: boolean): ColumnProps<RequirementData>[] => {
