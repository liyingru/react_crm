...
const CollectionCreateForm = Form.create({ name: 'form_in_modal' })(
    ...
    class extends React.Component {
        ...
        render() {
            const { visible, onCancel, onCreate, form, dataAarry, selectIdClick, stateSelectClick, selectId, stateSelectId, orderDetail } = this.props;
            ...
            let showT = <span style={{ color: 'red' }}>{'*目前对于订单有效'}{dataAarry ? dataAarry?.length : '0'}{'条预约单，请确认是否推送下一位预约人'}</span>

            if (orderDetail?.reserve_confirm_count > 0) {
                showT = <span style={{ color: 'red' }}>{'*目前对于订单有效'}{dataAarry ? dataAarry?.length - 1 : '0'}{'条预约单，请确认是否推送下一位预约人'}</span>
            }
                        ...
                        <div >
                            ...
                            <Form.Item label="是否有效：" {...formItemLayout}>
                                {getFieldDecorator('valid', {
                                    ...
                                    <Radio.Group onChange={stateSelectClick} >
...
class OrderDetailsConfirmShopPage extends React.Component<OrderDetailsConfirmShopProps> {
    ...
    render() {
        const { visible, data, orderDetail } = this.props;
        return (
            <div>
                <CollectionCreateForm
                    ...
                    stateSelectClick={this.stateSelectClick}
                    selectId={this.state?.selectId}
                    stateSelectId={this.state?.stateSelectId}
