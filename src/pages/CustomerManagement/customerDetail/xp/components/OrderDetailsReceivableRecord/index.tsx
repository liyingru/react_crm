...
interface recordItemProps extends FormComponentProps {
  ...
  editDisable: boolean,
  showItemDetails: (model: PlansItemList) => void,
}
...
class recordItem extends Component<recordItemProps>{
  ...
  render() {
    const { recordItemModel, orderInfo, editDisable } = this.props;
        ...
        render: (id: any) => {
          ...
          if (itemModel) {
            if (itemModel?.check_status == 0) {
              ...
              return (
                <div>
                  {editDisable == false && orderInfo?.show_money_button && orderInfo?.phase !== 320 && <>
    ...
    return (
      ...
      <div className={styles.container}>
        ...
        <div style={{ padding: 0 }}>
          <Table size="small" scroll={{ x: 'max-content' }} columns={columns} dataSource={recordItemModel && recordItemModel.list} pagination={false} />
        </div>
      </div>
    );
  }
