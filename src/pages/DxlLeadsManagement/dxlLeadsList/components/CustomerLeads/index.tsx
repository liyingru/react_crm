...
import { Checkbox, DatePicker, Divider, Form, Icon, Input, message, Modal, Radio, Select, Spin, Switch, Table, Tabs } from 'antd';
...
import { DistributeCompany, ReceiveUser } from '../../data';
...
interface CustomerLeadsProps extends FormComponentProps {
  ...
  optionable?: number;
  //刷新客户和客户线索列表
  fun_refreshCustomerLeadsAll: Function;
  //刷新客户线索列表
...
class CustomerLeads extends Component<CustomerLeadsProps, CustomerLeadsState> {
  ...
  initCategoryId = (): string => {
    ...
    if (this.props.defaultCategoryId == undefined || this.props.defaultCategoryId == null || this.props.defaultCategoryId == '' || this.props.defaultCategoryId == 'null') {
      return this.props.config.category[0].id
    }
    let categoryId = config.category.find(value =>
      value.id == this.props.defaultCategoryId
    )?.id
    return (categoryId == undefined || categoryId == null || categoryId == '' || categoryId == 'null') ? this.props.config.category[0].id : categoryId
  }
  ...
  render() {
    const { form, config, customerLeadsListData } = this.props;
  ...
  renderCategoryTab() {
    const { config } = this.props;
    console.log("renderCategoryTab", this.state.categoryId)
    return (
      config.category && config.category.length > 0 ? < Tabs activeKey={this.state.categoryId + ''} onChange={this.changeCategory}>
        {
