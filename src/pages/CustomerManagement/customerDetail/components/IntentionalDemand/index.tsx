...
import { RequirementData, RequirementDataDetails, ConfigItem, ReqButton } from '@/pages/LeadsManagement/leadsDetails/data';
...
interface IntentionalDemandProps extends FormComponentProps {
  ...
  dispatch: Dispatch<
    Action<
      ...
      | 'customerDetailMode/createReqList'
      | 'customerDetailMode/updateReqList'
      | 'customerDetailMode/closeReq'
      | 'customerDetailMode/openReq'
      | 'customerDetailMode/router'
      | 'customerDetailMode/getXPFlowInfo'
      | 'customerDetailMode/updateReqLite'
      | 'customerDetailMode/distributeCompany'
      | 'customerDetailMode/getRecommendMerchants'
      | 'customerDetailMode/submitAdvancedForm'
      | 'customerDetailMode/turnTrue'
    >
  >;
  loading: boolean;
...
class IntentionalDemand extends Component<IntentionalDemandProps, IntentionalDemandState> {
  ...
  fetchCustomerDetail = () => {
    const { dispatch, customerId, reqId, showStyle } = this.props;
    const params = {
  ...
  //创建有效单对话框确认
  handleCreateReqOk = () => {
    const { leads_id: leadsId } = this.props.customerDetailMode.data.reqLiteData
    const { dispatch, form, customerDetailMode, customerId } = this.props;
    const { data: { customerData: customer }, config: customerConfig } = customerDetailMode;
    ...
    form.validateFieldsAndScroll(formFeilds, (err, values) => {
      ...
      if (dressType != undefined) {
        let types: string = "";
        console.log("dressType = " + JSON.stringify(dressType));
      ...
      if (this.state.optionCategoryType == 0) { //创建
        ...
        console.log(params)
        dispatch({
          type: 'customerDetailMode/createReqList',
          payload: params,
        ...
        console.log(params)
        dispatch({
          type: 'customerDetailMode/updateReqList',
          payload: params,
          callback: (code: number, msg: string) => {
            if (code != 200) {
              return;
  ...
  handleOpenReq = (data: RequirementData) => {
    ...
    dispatch({
      type: 'customerDetailMode/openReq',
      payload: params,
      callback: (code: number, msg: string) => {
        if (code != 200) {
          return;
  ...
  //关闭有效单确认
  handleCloseReqOk = () => {
    const { form, dispatch, customerDetailMode } = this.props;
    const { data: { customerData: customer } } = customerDetailMode;
    ...
    form.validateFields(formFeilds, (err, values) => {
      ...
      dispatch({
        type: 'customerDetailMode/closeReq',
        payload: params,
        callback: (code: number, msg: string) => {
          if (code != 200) {
            return;
  ...
  handleRecommend = (data: RequirementData) => {
    console.log('customer data=======' + data.top_category, data)
  ...
  handleButton = (record: RequirementData, button: ReqButton) => {
    ...
    if (button.type == 10 || button.type == 20 || button.type == 21) {
      ...
      dispatch({
        type: 'customerDetailMode/getXPFlowInfo',
        payload: params,
  ...
  handleButtonOk = () => {
    ...
    if (this.state.buttonType == 10) {
      ...
      form.validateFields(formFeilds, (err, values) => {
        ...
        dispatch({
          type: 'customerDetailMode/turnTrue',
          payload: params,
      ...
      form.validateFields(formFeilds, (err, values) => {
        ...
        dispatch({
          type: 'customerDetailMode/distributeCompany',
          payload: params,
          callback: (success: boolean) => {
            if (success) {
              //关闭弹窗和重制
      ...
      form.validateFields(formFeilds, (err, values) => {
        ...
        dispatch({
          type: 'customerDetailMode/submitAdvancedForm',
          payload: params,
          callback: (success: boolean) => {
            if (success) {
              //关闭弹窗和重制
  ...
  searchMerchants = (key: string) => {
    const { dispatch } = this.props;
    dispatch({
      type: 'customerDetailMode/getRecommendMerchants',
      payload: {
      ...
      callback: (success: boolean) => {
        if (success) {
          ...
          const { customerDetailMode: { merchantsListSinglePageData, merchantsTotal } } = this.props;
  ...
  render() {
    const { loading, customerDetailMode, form, isclaimFlag } = this.props;
    const { data: { requirementData: reqGroupData }, config: customerConfig } = customerDetailMode;
  ...
  renderHaveCategory2() {
    const { customerDetailMode } = this.props;
    const { config: customerConfig } = customerDetailMode;
    const categorys = customerConfig && customerConfig.category2.filter(value => (Number(value.value) == this.state.optionCategoryValue));
    return (
  ...
  renderCategory2(category2: ConfigListItem[]) {
    const { form } = this.props;
    const { getFieldDecorator } = form;
  ...
  hyMyColumns: ColumnProps<RequirementData>[] = [
    {
      ...
      render: (text, record) => (
        <Fragment>
          {
            (this.props.readOrWrite == 1 && this.props.showStyle == 0) ? <a onClick={() => this.handleCustomerDetails(record)}>{text}</a> : <div>{text}</div>
          }
        </Fragment>
      )
    },
  ...
  rowClassName = (recoder: RequirementData) => {
    return recoder.id.toString() == this.props.reqId ? styles.clickRowStyl : ''
  }

  //婚宴
  renderHYTable = (name: string, datas: RequirementData[], isMy: boolean) => {
    const { readOrWrite } = this.props;
    const columns =
      readOrWrite == 0
        ? (isMy
          ? this.hyMyColumns.slice(0, this.hyMyColumns.length - 1)
          : this.hyColumns.slice(0, this.hyColumns.length - 1)
        )
        : (isMy ? this.hyMyColumns : this.hyColumns)
    return (
      <Table
  ...
  renderHQTable = (name: string, datas: RequirementData[], isMy: boolean) => {
    ...
    const columns =
      readOrWrite == 0
        ? (isMy
          ? this.hqMyColumns.slice(0, this.hqMyColumns.length - 1)
          : this.hqColumns.slice(0, this.hqColumns.length - 1)
        )
        : (isMy ? this.hqMyColumns : this.hqColumns)
    return (
      <Table
        scroll={{ x: 'max-content' }}
        size={"middle"}
  ...
  renderHSSYTable = (name: string, datas: RequirementData[], isMy: boolean) => {
    ...
    const columns =
      readOrWrite == 0
        ? (isMy
          ? this.hssyMyColumns.slice(0, this.hssyMyColumns.length - 1)
          : this.hssyColumns.slice(0, this.hssyColumns.length - 1)
        )
        : (isMy ? this.hssyMyColumns : this.hssyColumns)
    return (
      <Table
        scroll={{ x: 'max-content' }}
        size={"middle"}
  ...
  renderQDTable = (name: string, datas: RequirementData[], isMy: boolean) => {
    ...
    const columns =
      readOrWrite == 0
        ? (isMy
          ? this.qdMyColumns.slice(0, this.qdMyColumns.length - 1)
          : this.qdColumns.slice(0, this.qdColumns.length - 1)
        )
        : (isMy ? this.qdMyColumns : this.qdColumns)
    return (
      <Table
        scroll={{ x: 'max-content' }}
        size={"middle"}
  ...
  renderHCTable = (name: string, datas: RequirementData[], isMy: boolean) => {
    ...
    const columns =
      readOrWrite == 0
        ? (isMy
          ? this.hcMyColumns.slice(0, this.hcMyColumns.length - 1)
          : this.hcColumns.slice(0, this.hcColumns.length - 1)
        )
        : (isMy ? this.hcMyColumns : this.hcColumns)
    return (
      <Table
        scroll={{ x: 'max-content' }}
        size={"middle"}
  ...
  renderYZSTable = (name: string, datas: RequirementData[], isMy: boolean) => {
    ...
    const columns =
      readOrWrite == 0
        ? (isMy
          ? this.yzsMyColumns.slice(0, this.yzsMyColumns.length - 1)
          : this.yzsColumns.slice(0, this.yzsColumns.length - 1)
        )
        : (isMy ? this.yzsMyColumns : this.yzsColumns)
    return (
      <Table
        scroll={{ x: 'max-content' }}
        size={"middle"}
  ...
  renderHSLFTable = (name: string, datas: RequirementData[], isMy: boolean) => {
    ...
    const columns =
      readOrWrite == 0
        ? (isMy
          ? this.hslfMyColumns.slice(0, this.hslfMyColumns.length - 1)
          : this.hslfColumns.slice(0, this.hslfColumns.length - 1)
        )
        : (isMy ? this.hslfMyColumns : this.hslfColumns)
    return (
      <Table
        scroll={{ x: 'max-content' }}
        size={"middle"}
  ...
  renderEdit = (name: string, type: number, record: RequirementData, isMy: boolean, options: any[]) => {
    const { customerDetailMode } = this.props;
    const { permission } = customerDetailMode;
    const editotherreq = permission ? permission.editotherreq : false;
    if ((record.phase == '1' || record.phase == '2' || record.phase == '6') && (isMy || editotherreq) && (record.req_num != undefined && record.req_num != null && record.req_num != '')) {
  ...
  renderOpen = (record: RequirementData, isMy: boolean, options: any[]) => {
    const { customerDetailMode } = this.props;
    const { permission: { openotherreq, requirementadapter_openreq } } = customerDetailMode;
    if (((record.phase == '0' || record.phase == '5') && ((isMy && requirementadapter_openreq) || (!isMy && openotherreq)))) {
  ...
  renderClose = (record: RequirementData, isMy: boolean, options: any[]) => {
    const { customerDetailMode } = this.props;
    const { permission: { closeotherreq, requirementadapter_closereq } } = customerDetailMode;
    if (((record.phase == '1' || record.phase == '2' || record.phase == '6') && ((isMy && requirementadapter_closereq) || (!isMy && closeotherreq)))) {
  ...
  renderRecommend = (record: RequirementData, isMy: boolean, options: any[]) => {
    const { customerDetailMode } = this.props
    const { permission: { recommendotherreq, recommendreq } } = customerDetailMode;
    if ((record.phase == '1' || record.phase == '2' || record.phase == '6') && ((isMy && recommendreq) || (!isMy && recommendotherreq))) {
  ...
  renderCloseAlert = (record: RequirementData, isMy: boolean, options: any[]) => {
    const { customerDetailMode } = this.props
    const { permission: { viewotherreq } } = customerDetailMode;
    if (record.phase == '0' && (isMy || viewotherreq)) {
      options.push(<a style={{ color: 'grey' }} onClick={() => this.handleCloseReqAlert("关闭原因", record)}>关闭原因</a>);
    } else if (record.phase == '5' && (isMy || viewotherreq)) {
