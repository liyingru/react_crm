...
import { EmployeeQuarterlySalesTarget } from "../../salesTarget/data";
const { Option } = Select;

interface EmployeeQuarterlyTargetInputProps extends FormComponentProps {
  companyId: string,
  ...
  visible: boolean,
  onStructureChange?: (value: string, option?: React.ReactElement<any> | React.ReactElement<any>[]) => void;
  onOk: ((fieldsValue: any, targetId?: string) => void),
  onCancel?: (() => void)
}

const EmployeeQuarterlyTargetInput: React.FC<EmployeeQuarterlyTargetInputProps> = props => {
  const { companyId, structureValue, staffValue, structureList, staffList, targetModel, visible, onStructureChange, onOk, onCancel, form } = props;
    ...
    return (
      ...
      >
        <Form labelCol={{ span: 5 }} wrapperCol={{ span: 18 }} layout="horizontal">
          ...
          <Form.Item label="目标员工">
            {
              form.getFieldDecorator('staffId', { initialValue: targetModel?.user_id ?? staffValue, rules: [{ required: true, message: '请选择员工' }], })(
                <Select
                  showSearch
                  style={{ width: '100%' }}
                  placeholder="请选择员工"
                  ...
                  {staffList && staffList.map((staff) => (
                    <Option value={staff.id} key={staff.name}>{staff.name}</Option>
                  ))}
                </Select>
              )
            }
...
export default Form.create<EmployeeQuarterlyTargetInputProps>()(EmployeeQuarterlyTargetInput);
