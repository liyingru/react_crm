...
import OrderDetailsInputPerCent from "@/pages/OrderManagement/orderDetails/components/OrderDetailsInputPerCent";
...
class ContractTab extends Component<ContractTabPros, ContractTabState>{
    ...
    operateMethod = (recoder: any) => {
        ...
        if (recoder.audit_status == 0) {//待提交：提交和补充协议
            if (this.currentUserInfo.company_tag == 'XZ' || this.currentUserInfo.company_tag == 'SEW') {
                return (
                    <span>
                        <a onClick={() => this.editContract(recoder)}>重新编辑</a>|
                        <a onClick={() => this.submitContractAudit(recoder)}>提交审核</a>|
                        <label style={{ color: "gray" }}>录占比</label>
                    </span>
                )
            } else {
                return (
    ...
    editContract = (recoder: any) => {
        if (recoder.id) {
            ...
            this.props.dispatch(routerRedux.push({
                pathname: '/order/orderManagement/editContract',
                query: {
            ...
            <div>
                {
                    showDetail == false ? (
                        ...
                        (<div>
                            <Button onClick={() => this.backList()}><Icon type="left" />返回上级</Button>
