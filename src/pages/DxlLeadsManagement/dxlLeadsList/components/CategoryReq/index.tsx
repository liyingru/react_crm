...
import { Checkbox, DatePicker, Divider, Form, Icon, Input, message, Modal, Radio, Select, Table, Spin, Switch, Tabs, Menu } from 'antd';
...
import { Permission, ReceiveUser, DistributeCompany } from '../../data';
...
class CategoryReq extends Component<CategoryReqProps, CategoryReqState> {
  ...
  constructor(props: Readonly<CategoryReqProps>) {
    ...
    this.state = {
      ...
      categoryId: (this.props.config.category && this.props.config.category.length > 0) ? this.initCategoryId() : undefined,

      logModalVisible: false,
      logModalLeadId: '',

      appealModalVisible: false,
      appealReason: '',
    }
  }
  ...
  initCategoryId = (): string => {
    ...
    if (this.props.defaultCategoryId == undefined) {
      return this.props.config.category[0].id
    }
    let categoryId = config.category.find(value =>
      value.id == this.props.defaultCategoryId
    )?.id
    return categoryId == undefined ? this.props.config.category[0].id : categoryId
  }
  ...
  //创建有效单对话框确认
  handleCreateReqOk = () => {
    const { form, leadsId, customerId, config } = this.props;
  ...
  handleSelectCategory = (name: string, value: number) => {
    if(this.props.checkCustomerInfo && this.props.checkCustomerInfo() == false) {
      return;
    }
    this.setState({
  ...
  handleAppeal = (data: RequirementData) => {
    this.setState({
      editReq:data
    })
    this.setAppealVisible(true)
  }

  setAppealVisible = (visible:boolean) => {
    if(visible) this.setState({appealReason:''});
    this.setState({
      appealModalVisible:visible
    })
  }

  appealModeTextChange = ({ target: { value } }) => {
    this.setState({ appealReason: value });
  };

  onAppealModeSure = () => {
    if (this.state.appealReason == '') {
      message.error("请填写申诉原因");
      return;
    }
    this.setState({
      alertLoading: true,
    ...
    let params = {
      reqId: this.state.editReq?.id,
      reason:this.state.appealReason
    }
    message.info(params)
    // Axios.post(URL.reserveReq, params).then(
    //   res => {
  ...
  render() {
    const { form, config, reqGroupDetails } = this.props;
      ...
      <div>
        <Spin spinning={this.state.optionLoading}>
          <div style={{
                ...
                <Form>
                  ...
                  }
                  {
                    (this.state.optionCategoryValue == 1 || this.state.optionCategoryValue == 2 || this.state.optionCategoryValue == 6 || this.state.optionCategoryValue == 4) ? <FormItem label={(this.state.optionCategoryValue == 1 || this.state.optionCategoryValue == 2 || this.state.optionCategoryValue == 6) ? "婚礼桌数" : "举办桌数"} labelCol={{ span: 5 }} wrapperCol={{ span: 19 }} >
                      <NumberRangeInput style={{ width: '100%', marginLeft: 5 }} myForm={form} minimumField={'hotelTablesFrom'} maximumField={'hotelTablesEnd'} minimumValue={this.state.editReq ? (this.state.editReq.hotel_tables_from != '' ? Number(this.state.editReq.hotel_tables_from) : undefined) : undefined} maximumValue={this.state.editReq ? (this.state.editReq.hotel_tables_end != '' ? Number(this.state.editReq.hotel_tables_end) : undefined) : undefined} />
                    </FormItem> : null
                  }
                  {
                  ...
                  }
                  {
                    (this.state.optionCategoryValue == 1 || this.state.optionCategoryValue == 2 || this.state.optionCategoryValue == 4 || this.state.optionCategoryValue == 6) ? <FormItem label="每桌预算" labelCol={{ span: 5 }} wrapperCol={{ span: 19 }} >
                      <NumberRangeInput style={{ width: '100%', marginLeft: 5 }} myForm={form} minimumField={'perBudgetFrom'} maximumField={'perBudgetEnd'} minimumValue={this.state.editReq ? (this.state.editReq.per_budget_from != '' ? Number(this.state.editReq.per_budget_from) : undefined) : undefined} maximumValue={this.state.editReq ? (this.state.editReq.per_budget_end != '' ? Number(this.state.editReq.per_budget_end) : undefined) : undefined} />
                    </FormItem> : null
                  }
                  {
                    <FormItem label={this.state.optionCategoryValue == 1 ? "婚宴预算" : this.state.optionCategoryValue == 2 ? "婚庆预算" : this.state.optionCategoryValue == 3 ? "婚纱摄影预算" : this.state.optionCategoryValue == 4 ? `${React.$celebrationOrWeddingBanquet()}预算` : this.state.optionCategoryValue == 5 ? "婚车预算" : this.state.optionCategoryValue == 6 ? "整体预算" : this.state.optionCategoryValue == 7 ? "婚服预算" : ""} labelCol={{ span: 5 }} wrapperCol={{ span: 19 }} >
                      <NumberRangeInput style={{ width: '100%', marginLeft: 5 }} myForm={form} minimumField={'budgetFrom'} maximumField={'budgetEnd'} minimumValue={this.state.editReq ? (this.state.editReq.budget_from != '' ? Number(this.state.editReq.budget_from) : undefined) : undefined} maximumValue={this.state.editReq ? (this.state.editReq.budget_end != '' ? Number(this.state.editReq.budget_end) : undefined) : undefined} />
                    </FormItem>
                  }
            ...
            <Modal
              title='申诉原因'
              visible={this.state.appealModalVisible}
              centered
              destroyOnClose={true}
              okButtonProps={{ disabled: this.state.alertLoading }}
              cancelButtonProps={{ disabled: this.state.alertLoading }}
              onOk={this.onAppealModeSure}
              onCancel={() => this.setAppealVisible(false)}>
              <Spin spinning={this.state.alertLoading}>
                <TextArea
                  onChange={this.appealModeTextChange}
                  placeholder='请填写申诉原因（必填）'
                  rows={6}>
                </TextArea>
                {
                  this.props.config.gender?.length > 0 &&
                  <div>
                    <div style={{fontWeight:'bold',marginTop:10}}>申诉历史:</div>
                    {
                      this.props.config.gender?.map(item => <div style={{marginTop:5}}>{item}</div>)
                    }
                  </div>
                }
              </Spin>
        ...
        <Tabs activeKey={this.state.categoryId + ''} onChange={this.changeCategory}>
          {
            config.category.map(categ => (
              <TabPane tab={categ.name} key={categ.id} />
            ))
          }
        </Tabs >
      )
